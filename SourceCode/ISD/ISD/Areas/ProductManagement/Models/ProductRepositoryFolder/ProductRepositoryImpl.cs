...
namespace ISD.Areas.ProductManagement.Models.ProductRepositoryFolder
{
    ...
    {
        private const string GET_DATA = "SELECT p.*, c.NAME, ps.QUANTITY, ps.NUMOFSOLD, ps.NUMOFVIEW,"
        ...
        private const string CREATE= "INSERT INTO PRODUCTS(CATEGORYID, NAME, PRICE, [DESCRIPTION], CREATEDBY, CREATEDDATE)"
            + " VALUES(@CATEGORYID, @NAME, @PRICE, @DESCRIPTION, @CREATEDBY, @CREATEDDATE)";
        private const string UPDATE= "UPDATE PRODUCTS SET NAME = @NAME, PRICE = @PRICE, [DESCRIPTION] = @DESCRIPTION,"
        ...
        private Products toObject(DataRow dr)
        {
            return new Products() {
                ...
                modifiedBy = Int16.Parse(dr["MODIFIEDBY"].ToString()),
                modifiedByAccount = dr["MODIFIEDBYACCOUNT"].ToString(),
                modifiedDate = DateTime.Parse(dr["MODIFIEDDATE"].ToString())
            };
        }
    }
}